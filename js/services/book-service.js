...
function searchGoogleBook(searchStr) {
  const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchStr}`
  const googleBooks = [];
  storageService.get(GOOGLE_BOOKS_KEY)
    .then(books => {
  ...
  storageService.query(BOOKS_KEY)
    .then(books => {
      if (!books || books.length) googleBooks = []
      else googleBooks = books
    })
  return Promise.resolve()
  return axios.get(url)
    .then(res => {
      //TODO - save to local storage the result
...
function addGoogleBook(googleBook) {
  ...
  const { title, publisher } = googleBook.volumeInfo
  const formattedBook = {
    id: googleBook.id,
    title,
    publisher
  ...
  storageService.post(BOOKS_KEY, formattedBook)
}
